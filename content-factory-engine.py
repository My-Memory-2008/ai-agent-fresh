...
# Safety default fallback templates to insulate pipeline runs from API timeouts
seo_metadata = {
    "title": "Most Oddly Satisfying ASMR Challenge! 🤯 #shorts",
    "description": "Wait till the end for the funny cat reaction loop! Original concept inspired by account creator. #shorts #asmr",
...
if groq_key:
    try:
        ...
        clean_json_text = chat_completion.choices.message.content.strip().replace('```json', '').replace('```', '').strip()
...
# ==========================================
# 4c. DATASET REACTION LOADER & AUTOMATED SPEED SYNC
# ==========================================
...
print(f"🚀 GPU Rendering Finished! Asset Compiled at: {OUTPUT_VIDEO}")
...
try:
    ...
    yt_url = f"youtube.com{response['id']}"
    upload_success = True
...
try:
    ...
    led_url = f"github.com{GITHUB_USER}/{GITHUB_REPO}/contents/reel_queue.json"
    ...
    for entry in current.get('processed', []):
        if entry['url'] == reel_url and entry.get('status') == 'in_progress':
            ...
            if yt_url:
                entry['youtube_url'] = yt_urlentry['completed_at'] = datetime.now(timezone.utc).isoformat()
                break
            new_content = requests.utils.b64encode(json.dumps(current).encode()).decode()
            requests.put(led_url, headers=headers_gh, 
                         json={"message": "Auto: Updated reel status", 
                               "content": new_content,
                               "sha": resp_gh.json()["sha"]})
