...
# ==========================================
# PHASE A: PART 1 OF 2 (FULL-SCREEN AI MULTI-CREATOR SCANNER CORE)
# ==========================================
print("🧠 Launching Full-Screen Gemini Multi-Creator Palette Scanner...")
...
# --- 1. CAPTURE STRUCTURAL SCREEN PARAMETERS ---
...
# 🔥 FIXED POLYGON MATRIX DEFINITION LAYER:
# Opens up the structural search window to 100% of the video canvas with pristine bracket formatting arrays
min_x_fallback = 0
max_x_fallback = orig_width
min_y_fallback = 0
max_y_fallback = orig_height
polygon_vertices = np.array([[0, 0], [orig_width, 0], [orig_width, orig_height], [0, orig_height]], dtype=np.int32)
...
if openrouter_key and ret_v:
    try:
        ...
        headers = {
            ...
            "X-Title": "Universal Full-Screen Scanner"
        }
        ...
        print(f"⚠️ Intelligence lane processing challenge encountered: {vision_fault}")

# --- 2. FULL-SCREEN NUMERIC SAMPLING & DYNAMIC SECTOR ANCHOR ---
if ret_v:
    roi_pixels = sample_frame[int(orig_height*0.4):int(orig_height*0.6), int(orig_width*0.3):int(orig_width*0.7)]
    ...
    contours_s, _ = cv2.findContours(local_edges_sample, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    sample_x_points, sample_y_points = [], []
    
    for cnt in contours_s:
        sx, sy, sw, sh = cv2.boundingRect(cnt)
        if sw >= 3 and sh >= 4 and sw < 100 and sh < 40 and sy > (orig_height * 0.5):
    ...
    if sample_x_points and sample_y_points:
        ...
        fixed_cy = int(orig_height * 0.88) - 4
else:
    avg_b, avg_g, avg_r = 240, 240, 240
    text_color, shadow_color = (255, 255, 255), (15, 15, 15)
    fixed_cx, fixed_cy = int(orig_width * 0.5), int(orig_height * 0.88)

INPUT_REEL = output_path
FINAL_MONETIZED_OUTPUT = "/kaggle/working/final_monetized_output.mp4"
print(f"🔒 Full-screen anchor grid locked live -> Center X: {fixed_cx} | Center Y: {fixed_cy}")
